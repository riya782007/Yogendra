...
describe("canonical colour snap (bulk-upload typos)", () => {
  ...
  it("maps gren to Green, not Grey, and never creates a new colour", () => {
    for (const s of ["gren", "Gren", "GREN", "grn", "greeen"]) {
      expect(canonicalColorName(s), s).toBe("Green");
    }
    expect(canonicalColorName("Grey")).toBe("Grey");
    expect(canonicalColorName("gray")).toBe("Grey");
    expect(isRedundantColorOption("gren")).toBe(true);
    expect(barcodeCodeForColor("gren")).toBe("GREEN");
  });
