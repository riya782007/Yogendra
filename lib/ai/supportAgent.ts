...
const SYSTEM =
  ...
  `links; explain pricing, offers, COD, and shipping (flat ₹100 shipping per order, Cash on Delivery available); ` +
...
export async function runSupportAgent(ctx: AgentContext): Promise<AgentReply> {
  ...
  const user = [
    ...
    ``,
    `POLICIES: Flat ₹100 shipping on every order. Cash on Delivery available. Premium anti-tarnish artificial jewellery.`,
    hist ? `\nRECENT CONVERSATION:\n${hist}` : ``,
    ``,
