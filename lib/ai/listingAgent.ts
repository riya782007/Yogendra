...
function prompt(p: ProductLike) {
  ...
  return [
    ...
    ``,
    `DESCRIPTION — the owner wants it SIMPLE: NO opening paragraph and NO marketing prose. It is ONE string of EXACTLY five labelled blocks, each on its own line separated by a real newline "\\n", in this exact order. Only BOX CONTAINING and MATERIAL & CRAFTSMANSHIP change per product; the other three are fixed verbatim:`,
    `    "BRAND: BlytheDIVA"`,
    `    "BOX CONTAINING: {the main piece + ONLY the extra pieces THIS product actually has}." — Rules: necklace alone → "One necklace." ; necklace + earrings → "One necklace with a pair of earrings." ; add "and a maang tikka" ONLY if the name/specs mention a Maang Tikka ; add "and a finger ring" ONLY if it has a ring. A Necklace Set or Choker Set includes a pair of earrings. A standalone earring pair → "A pair of earrings." NEVER list a piece the name/specs/photo do not show.`,
    `    "MATERIAL & CRAFTSMANSHIP: Finest quality {ONLY the material(s) actually used — e.g. American Diamond, Kundan, Pearls, Meenakari, Polki, Moissanite, Temple work, Turkish Stone, Crystal, Oxidised finish, Stones — joined naturally} with environment-friendly non-precious metals are used to make {this necklace / these earrings / this choker / this ring}. It is safe to wear for long hours."`,
    ...
    `  CRITICAL: BOX CONTAINING lists ONLY real pieces and MATERIAL & CRAFTSMANSHIP lists ONLY real materials, taken from the name, specifications or photo. Never invent a piece (no maang tikka / ring / earrings unless present) or a material (no Kundan / American Diamond / Pearls unless present). Output NOTHING beyond these five blocks — no opening sentence, no "ideal for weddings", no closing line.`,
    ``,
