...
function prompt(p: ProductLike) {
  ...
  const isNathHint = /\bnath\b|nathni|nose ?pin|nosepin/i.test(`${p.name} ${p.categoryName ?? ""} ${(p as any).subcategoryName ?? ""} ${kw}`);
  return [
    ...
    `  • The CATEGORY and SUB-CATEGORY the owner selected guide the STYLING and are usually the type (e.g. Category "Earrings" + Sub-category "Kundan Earrings" → a Kundan earring; Sub-category "Temple Necklace" → a temple-style necklace). BUT the PHOTO is the ground truth for WHAT THE ITEM ACTUALLY IS: when the photo clearly shows something more specific than a generic category label, name what you SEE — most importantly, if it has a DIAL it is a WATCH ("Watch" / "Bracelet Watch"), never a plain "Bracelet" or "Hand Accessory"; likewise a bangle/kada, anklet, nath/ nose pin, brooch or hair-accessory. Do not force a vague category word onto a piece the photo shows to be something specific. (This is about the correct TYPE only — you still must NEVER invent stones, motifs or extra pieces that aren't there.)`,
    `  • CRITICAL FOR NATH / NOSE PIN: If the name, category, sub-category, keywords OR the photo indicate a nath / nose pin / nose ring / nathni, the type MUST be "Nose Pin" or "Nath". NEVER call it "Pendant", "Pendant Chain", "Necklace", "Chain", or "Other Accessories". Category in specs = "Nose Pin". Work/Style = "Nose Pin" or the real material/style (e.g. "Kundan Nose Pin"). Box Containing = "One nose pin" (or "One nath"). Tags must include nath / nose pin / nose ring. Do NOT invent a chain or pendant.`,
    ...
    `  • A "Set" in the NAME does NOT mean the piece includes earrings or any second piece (a "Pendant Set" can be a single pendant). List earrings ONLY if the word earrings / jhumka / danglers literally appears; list a maang tikka / ring ONLY if named. If only one piece is named, "Box Containing" is just that one piece (e.g. "One pendant" or "One nose pin") and the description must NOT say "the matching earrings complete the set".`,
    ...
    isNathHint
      ? `• This product is a NATH / NOSE PIN. Title type must end with Nose Pin or Nath. Specs Category = "Nose Pin". Box Containing = "One nose pin". Work/Style must NOT be "Pendant Chain".`
      : ``,
    ``,
    lockedTitle
    ...
    ``,
    `specs (object) MUST include: Category, "Box Containing" (the exact pieces, e.g. "One necklace with a pair of earrings and a maang tikka" or "One nose pin" — only real pieces), Material (ONLY the material(s) actually used), Work/Style, Occasion, Care${colors ? ", Colours" : ""}. DO NOT include the SKU. For nath: Category = "Nose Pin", Box Containing = "One nose pin", Work/Style must not say Pendant Chain.`,
    `tags: 8-12 short search tags mixing type, style, material, occasion. For nath include: nath, nose pin, nose ring.`,
    `seo.metaTitle <= 60 chars (title + " | BlytheDIVA"); seo.metaDescription <= 155 chars, compelling; seo.keywords 8-12 long-tail phrases like "kundan necklace set for wedding", "artificial jewellery online India", "bridal jewellery Delhi", "nath for women".`,
...
export async function generateTitleOptions(p: ProductLike, n = 4): Promise<{ titles: string[]; provider: string; usedImage: boolean }> {
  ...
  const userPrompt = [
    ...
    `  • The product TYPE at the end MUST match what you actually SEE in the photo — a watch is a "Watch" / "Bracelet Watch", a bangle is a "Bangle", an anklet is an "Anklet", a nath is a "Nose Pin" / "Nath" — never mislabel it as a generic "Bracelet", "Pendant Chain" or "Hand Accessory" when the photo shows something more specific.`,
    ``,
