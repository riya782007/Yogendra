-- Counter bills store the rate the owner typed in the Rate box (higher or lower than catalogue).
-- Storefront checkout omits unit_price and is unchanged.
-- Optional unit_mrp is ONLY for an explicit Disc column (Rate → Disc → Amount). A typed-down
-- selling price is not a discount — it is the new Rate, same as Square / Shopify POS.
...
begin
  ...
  for it in select * from jsonb_array_elements(p_items) loop
    ...
    -- unit_mrp is a pre-discount Rate. Never keep it when it is not actually higher than the billed rate
    -- (a price increase, or a simple Rate edit, must print as Rate = unit_price with Disc blank).
