...
import { getEmployees, getEmployeePerformance, getEmployeeSalesLedger } from "@/lib/supabase/queries";
    ...
    <main className="p-4 sm:p-8 bg-cream/40 min-h-screen">
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-sand/60">
            {ledger.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-muted">No attributed sales in this period.</td></tr>
            )}
            {ledger.map((s) => (
              <tr key={s.id} className="hover:bg-cream/40">
                <td className="px-4 py-2.5 text-muted whitespace-nowrap">{new Date(s.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "2-digit" })}</td>
                <td className="px-4 py-2.5"><Link href={`/admin/invoice/${s.id}`} className="text-emerald nav-link font-medium">{s.invoice_no || s.id.slice(0, 8).toUpperCase()} ↗</Link></td>
