...
import { getInventoryClassified, getCategoryTreeCached } from "@/lib/supabase/queries";
...
export default async function Inventory({ searchParams }: { searchParams: { deadDays?: string; lowQty?: string; cls?: string; q?: string; page?: string } }) {
  ...
  const [rows, categoryTree] = await Promise.all([
  ...
  return (
    <main className="p-4 sm:p-8 bg-cream/40 min-h-screen">
      <div className="flex items-start justify-between gap-4 mb-1">
        ...
        <StockExportButton categories={categoryTree} />
      </div>
