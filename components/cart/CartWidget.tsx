...
export function CartWidget() {
  ...
  // Portal target: the drawer must escape the header's `backdrop-blur` wrapper, otherwise its
  // `position: fixed` is trapped inside the header box (a transformed/blurred ancestor becomes the
  // containing block) — which renders the bag as an overlapping mess instead of a full-screen drawer.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  // Lock background scroll while the drawer is open (standard drawer behaviour).
  useEffect(() => {
    if (!open) return;
  ...
  const drawer = (
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute right-0 top-0 h-full w-[88%] max-w-md bg-ivory shadow-luxe flex flex-col animate-slideInRight">
        <div className="flex items-center justify-between p-5 border-b border-sand shrink-0">
          ...
          <button aria-label="Close" onClick={() => setOpen(false)} className="text-xl text-muted hover:text-rose transition-colors">✕</button>
        </div>
        ...
        {items.length > 0 && (
          <div className="p-5 border-t border-sand shrink-0">
            ...
            <p className="text-xs text-muted mb-3">₹80 flat shipping · Pay online at checkout</p>
