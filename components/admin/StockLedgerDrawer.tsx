                  ...
                  <div className="rounded-xl border border-sand bg-white divide-y divide-sand/60">
                    {items.map((r) => (
                      <div key={r.id} className="p-2.5 flex items-center gap-3 text-sm">
                        <span title={r.kind === "reserve" ? "Set aside for a held estimate — release that estimate to return this piece to stock" : r.kind === "return" ? (String(r.source ?? "").toLowerCase().includes("cancel") ? "Order cancelled/rejected — its stock came back" : String(r.source ?? "").toLowerCase().includes("bill edit") ? "Bill qty reduced — the un-billed piece came back" : "A customer returned this piece") : undefined} className={`px-2 py-0.5 rounded-full text-[10px] capitalize ${KIND_STYLE[r.kind] ?? "bg-cream text-muted"}`}>{r.kind === "reserve" ? "reserved" : r.kind === "return" ? (String(r.source ?? "").toLowerCase().includes("cancel") ? "cancelled" : String(r.source ?? "").toLowerCase().includes("bill edit") ? "bill edit" : "return") : r.kind}</span>
