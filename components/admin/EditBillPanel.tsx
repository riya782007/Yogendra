...
export function EditBillPanel({ orderId, requireOtp = true }: { orderId: string; requireOtp?: boolean }) {
  ...
  async function addLine() {
    if (requireOtp && !otp.trim()) { setMsg({ text: "Enter the owner OTP first.", ok: false }); return; }
  ...
  if (!open) {
    return (
      <div className="bg-white rounded-2xl p-5 shadow-card">
        <h2 className="font-medium text-ink mb-1">✎ Edit this bill {requireOtp ? <span className="text-xs text-muted font-normal">· owner OTP required</span> : <span className="text-xs text-emerald-dark font-normal">· held COD — staff can correct qty</span>}</h2>
        <p className="text-xs text-muted mb-3">{requireOtp
          ? "Fix a wrong quantity, remove a mis-scanned line, or add the right one — e.g. swap a wrongly-picked colour. Stock & totals correct themselves; no need to cancel the whole bill."
          : "This Cash-on-Delivery order is still held. Change quantities or swap a colour now — sellable stock stays reserved for this customer until you confirm dispatch."}</p>
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-card sm:col-span-2">
      ...
      <p className="text-[11px] text-muted mt-2">{requireOtp
        ? "Note: this changes stock and the recorded sale. If money was already collected, settle any difference with the customer in cash — the bill will show the new balance."
        : "Note: this only changes the hold. Confirming dispatch later moves the reserved pieces into a sale."}</p>
    </div>
  );
}
