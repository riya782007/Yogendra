    ...
    <div className="max-w-3xl">
      ...
      <div className={`bg-white rounded-2xl p-6 shadow-card transition-opacity ${catId ? "" : "opacity-40 pointer-events-none"}`}>
        ...
        {mode === "single" ? (
          ...
          <div className="space-y-3">
            <p className="text-xs text-muted">Paste any list — even messy. The AI figures out names, prices, stock and colours. Or use the strict format: <code className="bg-cream px-1 rounded">name, base_price, qty, type, colours|pipe</code> · <a download="blythe-diva-bulk-template.csv" href={`data:text/csv;charset=utf-8,${encodeURIComponent("name,base_price,qty,type,colours\nRajwadi Kundan Necklace,850,12,configurable,Red|Green|Blue\nPearl Studs,160,40,simple,\nMeenakari Bangles,540,25,configurable,Red|Green")}`} className="text-emerald nav-link">⤓ Download CSV template</a></p>
